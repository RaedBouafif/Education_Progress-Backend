    ...
    try {
        ...
        try {
            ...
            if (findedSession.subTeacher) {
                notificationData = {
                    session: findedSession._id,
                    object: 'Une séance a été modifiée dans votre emploi du temps',
                    notificationType: "updateSession",
                    receivers: [{ receiverId: findedSession.teacher._id, receiverPath: "Teacher" }, { receiverId: findedSession.subTeacher._id, receiverPath: "Teacher" }, ...findedPlanning.group.students?.map((element) => ({ receiverId: element, receiverPath: "Student" }))],
                ...
                notificationData = {
                    ...
                    object: 'Une séance a été modifiée dans votre emploi du temps',
                    notificationType: "updateSession",
                    receivers: [{ receiverId: findedSession.teacher._id, receiverPath: "Teacher" }, ...findedPlanning.group.students?.map((element) => ({ receiverId: element, receiverPath: "Student" }))],
...
exports.toggleCancelSession = async (req, res) => {
    try {
        ...
        const updateSession = await Session.findOneAndUpdate(sessionId, { canceled: status }, { runValidators: true, new: true })
            ...
            .populate("classroom")
            .populate("subject")
        console.log(updateSession.group)
        if (updateSession) {
            try {
                var notificationData
                if (status == true) {
                    notificationData = {
                        ...
                        object: `Une séance a été annuler`,
                        ...
                        receivers: [{ receiverId: updateSession.teacher._id, receiverPath: "Teacher" }, ...(updateSession.group.students ? updateSession.group.students.map((element) => ({ receiverId: element, receiverPath: "Student" })) : [])],
                    ...
                    notificationData = {
                        session: updateSession._id,
                        object: `Une séance a été activer`,
                        content: `Prof : ${updateSession.teacher.firstName + " " + updateSession.teacher.lastName} \n Salle : ${updateSession?.classroom?.classroomName} \n Matiere : ${updateSession?.subject?.subjectName} \n Type séance : ${updateSession.sessionType} \n Durée de séance : ${transformToTime(updateSession?.startsAt)}H - ${transformToTime(updateSession?.endsAt)}H`,
                        receivers: [{ receiverId: updateSession.teacher._id, receiverPath: "Teacher" }, ...(updateSession.group.students ? updateSession.group.students.map((element) => ({ receiverId: element, receiverPath: "Student" })) : [])],
                        notificationType: "cancelSession"
                    }
